...
from linepy import *
import time,random,sys,json,codecs,threading,glob,re,os,subprocess,tempfile
...
ow = LineClient(authToken="EoPZoZLXPfJMIeFlJNI7.mMD8f775uswF2ssZgJVO5W.H1Om8DTtC0vA5i96Zw23M6Ea3bVNxGiuFLureKmRVO8=")
ow.log("Auth Token : " + str(ow.authToken))
ow_channel = LineChannel(ow)
ow.log("Channel Access Token : " + str(ow_channel.channelAccessToken))
ow.poll = LinePoll(ow)
...
# global botenMode
# botenMode = False
DEVELOPER_KEY = 'AIzaSyAUYbyMn1_ViUuF9UnqFce_U5eeHQg5Ti8'
...
#-----------------------------------------------
    ...
    #-------------Fungsi Set Joined Battle of Ten Finish---------------#
        ...
        #-------------Fungsi Youtube Start-----------------------#
              ...
              # if msg.from_ in admin:
                yt_txt = msg.text.replace(".youtube ","")
                parser = argparse.ArgumentParser()
                parser.add_argument('--q', help='Search term', default=yt_txt)
                parser.add_argument('--max-results', help='Max results', default=1)
                args = parser.parse_args()
                print args

                try:
                  do_search=youtube_search(args)
                  cl.sendText(msg.to,do_search)
                except HttpError, e:
                  print 'An HTTP error %d occurred:\n%s' % (e.resp.status, e.content)
        #-------------Fungsi Youtube Finish----------------------#

        #-------------Fungsi Memebuzz Start-----------------------#
            elif ".memebuzz " in msg.text:
              if msg.from_ in owner:
                meme_txt = msg.text.replace(".memebuzz ","")
                text=meme_txt.split("|")
                print text
                ...
                print genMeme
                cl.sendImage(msg.to,genMeme)
        #-------------Fungsi Memebuzz Finish----------------------#
        ...
        #-------------Fungsi Auto Join Battle Start-----------------------#
              ...
              ki.sendText(wait["joinedBoT"], (settxt))
...
def youtube_search(options):
  ...
  # print "\n"
  if videos != []:
    result = "https://youtube.com/watch?v="+search_result['id']['videoId']
  elif channels != []:
    result = "https://youtube.com/channel/"+search_result['id']['channelId']
  return result
...
while True:
    try:
        Ops=ow.poll.trace(threading=True)
    except EOFError:
        raise Exception("It might be wrong revision\n" + str(cl.Poll.rev)+" "+str(e))
    ...
    for Op in Ops:
        ...
        if (Op.type != OpType.END_OF_OPERATION):
            ...
            ow.poll.setRevision(Op.revision)
            bot(Op)
